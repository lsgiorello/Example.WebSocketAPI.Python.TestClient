...
# Let's get dirty
from event_queue import QueueWriter
queue_writer = QueueWriter('event_queue.dat')
...
def parse_args(args=None):
    ...
    parser.add_argument('-u', dest='user',
                        ...
                        default='harshit.kamboj@alpha-grep.com')
    ...
    parser.add_argument('-H', dest='host',
                        ...
                        default='eu-west-1-aws-1-sm.optimized-pricing-api.refinitiv.net')
    parser.add_argument('-ah', dest='authHostname',
                        help='authorization server hostname',
                        default='51.91.62.59') # hector
...
def get_sts_token():
    ...
    url = 'http://{}:{}/get_all_tokens'.format(opts.authHostname, opts.authPort)
    ...
    #[access_token, refresh_token, expiry] = str(r.content).split('\n')
    access_token, refresh_token, expiry = r.content.decode("utf-8").split('\n')
...
if __name__ == '__main__':
    ...
    def on_message(ws, message):
        length = len(message)
        #print(type(message), length, message)
        queue_writer.push(message)
        market_data.on_message(ws, message)

    ws_app = websocket.WebSocketApp(ws_address, header=['User-Agent: Python'],
                                        on_message= on_message, #market_data.on_message,
    ...
    try:
        ...
        reissue_token_time = time.time() + (int(expire_time) - 10)
        ...
        while (((opts.exitTimeMins==0) or (time.time() < end_time))
            ...
            if time.time() >= reissue_token_time:
                sts_token, refresh_token, expire_time = get_sts_token()
                if sts_token == "NONE":
                    print("Could not get authorisaton token")
                    break
